...
Trust: `{F:0.8,G:0.7,R:0.8}` `crystal spec` passed with 62 examples, `sh scripts/spec_check` passed, `make clean && make commander` passed, and `sh scripts/commanderctl state` returned JSON with `uri` fields; internal PanelState still stores local paths.
