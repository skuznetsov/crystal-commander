...
Trust: `{F:0.8,G:0.6,R:0.8}` `crystal spec` passed with 89 examples, `sh scripts/spec_check` passed, `shards build` passed, `make commander` passed, `scripts/tabs_smoke` passed, `scripts/vfs_smoke` passed, `scripts/ipc_smoke` passed, and automation specs covered valid IPC, structured command envelopes, read-only snapshot requests, malformed JSON, existing-path safety, mutating command denial, and dry-run mutating command allowance.
